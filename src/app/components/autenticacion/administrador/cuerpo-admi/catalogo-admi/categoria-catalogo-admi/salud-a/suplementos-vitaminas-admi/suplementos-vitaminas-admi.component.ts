...
@Component({
  selector: 'app-suplementos-vitaminas-admi',
  standalone: true,
  imports: [CabeceraAdmiComponent, PieAdmiComponent,FormsModule, CommonModule, CabeceraCatalogoAdmiComponent],
  templateUrl: './suplementos-vitaminas-admi.component.html',
  styleUrl: './suplementos-vitaminas-admi.component.css'
})
export class SuplementosVitaminasAdmiComponent {
  ...
  addCard() {
    const newProduct: Producto = {
      ...
      available: true,
      code: '',
      category: 'suplementos_vitaminas',
      isOnSale: false,
      isFeatured: false,
      isFavorite: false
    };
    ...
    this.productService.addProduct(newProduct).subscribe(
      (id) => {
        newProduct.id = id;
        this.suplementos_vitaminas.push(newProduct);
        this.filteredSuplementos_vitaminas = [...this.suplementos_vitaminas];
      },
      (error) => {
  ...
  removeCard(index: number) {
    const producto = this.suplementos_vitaminas[index];
    if (producto.id !== undefined) {
      this.productService.deleteProduct(producto.id).subscribe(
        () => {
          this.suplementos_vitaminas.splice(index, 1);
          this.filteredSuplementos_vitaminas = [...this.suplementos_vitaminas];
  ...
  saveCard(index: number) {
    ...
    this.productService.updateProduct(producto).subscribe(
      (updatedProduct) => {
        this.suplementos_vitaminas[index] = updatedProduct;
        this.suplementos_vitaminas[index].isEditing = false;
        this.filteredSuplementos_vitaminas = [...this.suplementos_vitaminas];
  ...
  onFileSelected(event: any, index: number) {
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (e: any) => {
        this.suplementos_vitaminas[index].image = e.target.result;
        this.filteredSuplementos_vitaminas = [...this.suplementos_vitaminas];
      };
