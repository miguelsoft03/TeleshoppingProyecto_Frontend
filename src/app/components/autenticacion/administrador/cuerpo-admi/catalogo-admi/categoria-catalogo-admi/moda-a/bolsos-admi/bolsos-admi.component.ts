...
@Component({
  selector: 'app-bolsos-admi',
  standalone: true,
  imports: [CabeceraAdmiComponent, PieAdmiComponent,FormsModule, CommonModule, CabeceraCatalogoAdmiComponent],
  templateUrl: './bolsos-admi.component.html',
  styleUrl: './bolsos-admi.component.css'
})
export class BolsosAdmiComponent {
  bolsos: any[] = [];
  filteredBolsos: any[] = [];
  searchTerm: string = '';
  sortOrder: 'asc' | 'desc' | '' = '';
  ...
  loadBolsos() {
    this.productService.getProductsByCategory('bolsos').subscribe(
      (data) => {
        this.bolsos = data;
        this.filteredBolsos = [...this.bolsos];
      },
      (error) => {
        console.error('Error al cargar los bolsos', error);
      }
    );
  }

  filterProducts() {
    this.filteredBolsos = this.bolsos.filter(producto =>
      producto.title.toLowerCase().includes(this.searchTerm.toLowerCase())
    );

    if (this.sortOrder === 'asc') {
      this.filteredBolsos.sort((a, b) => a.price - b.price);
    } else if (this.sortOrder === 'desc') {
      this.filteredBolsos.sort((a, b) => b.price - a.price);
    }
  }
  ...
  editProduct(index: number) {
    this.bolsos[index].isEditing = true;
  }
  ...
  saveCard(index: number) {
    ...
    this.productService.updateProduct(producto).subscribe(
      (updatedProduct) => {
        this.bolsos[index] = updatedProduct;
        this.bolsos[index].isEditing = false;
        this.filteredBolsos = [...this.bolsos];
  ...
  onFileSelected(event: any, index: number) {
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (e: any) => {
        this.bolsos[index].image = e.target.result;
        this.filteredBolsos = [...this.bolsos];
      };
